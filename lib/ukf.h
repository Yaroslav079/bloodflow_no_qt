...
public:
    ...
    void set_up_task(Task &task, const Eigen::Vector<double, 7> &Teta);
    static void set_up_task(Task &task, const Eigen::Vector<double, 7> &Teta, const std::string blood_config, const std::string heart_config, const std::string base_path, Rescaler &rescaler);
...
private:
    ...
    const static int p = 7; // number of params
    const static int k = 3; // number of target variables
    const double sigma_param_sqr = 5e-3;
